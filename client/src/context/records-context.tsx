...
export const RecordsProvider = ({children} : {children:React.ReactNode;}) => {
    ...
    const fetchRecords = async () => {
        if (!user) return;
        const response = await fetch(`https://financetrackerapi.vercel.app/records/getAll/${user.id}`);
        if (response.ok) {
    ...
    const addRecord = async (record: Records) => {
        const response = await fetch("https://financetrackerapi.vercel.app/records", {
            method: "POST",
    ...
    const deleteRecord = async (id: string) => {
        const response = await fetch(`https://financetrackerapi.vercel.app/records/${id}`, {
            method: "DELETE",
        });
        try {
            if (response.ok) {
